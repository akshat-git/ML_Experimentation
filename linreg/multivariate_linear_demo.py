...
def make_data():
    ...
    base = rng.normal(0, 2.0, 90)
    x1 = base + rng.normal(0, 0.5, 90)
    x2 = 0.7 * base + rng.normal(0, 0.5, 90)
    X = np.column_stack([x1, x2])
    y = 1.0 + 3.1 * x1 - 2.2 * x2 + rng.normal(0, 1.35, 90)
    X = X - X.mean(axis=0)
    y = y - y.mean()
    return X, y
...
def fit_model(X, y):
    ...
    for _ in range(75):
        theta = gradient_step(theta, X, y, 0.07)
...
def make_contour(X, y, history):
    w1_values = np.linspace(history[:, 0].min() - 1.5, history[:, 0].max() + 1.5, 120)
    w2_values = np.linspace(history[:, 1].min() - 1.5, history[:, 1].max() + 1.5, 120)
...
def plot_results(X, y, theta, history):
    ...
    axes[0].scatter(y, predictions, color="seagreen", alpha=0.8)
    ...
    axes[0].set_title("Multivariate regression")
    axes[0].set_xlabel("actual y")
    axes[0].set_ylabel("predicted y")

    contour = axes[1].contour(mesh_x, mesh_y, grid, levels=25, cmap="magma")
    axes[1].clabel(contour, inline=True, fontsize=7, fmt="%.1f")
    axes[1].plot(history[:, 0], history[:, 1], marker="o", markersize=3, color="darkorange")
